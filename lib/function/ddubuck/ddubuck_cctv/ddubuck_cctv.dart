...
class CCTVMapScreen extends StatefulWidget {
  @override
  _CCTVMapScreenState createState() => _CCTVMapScreenState();
}
...
class _CCTVMapScreenState extends State<CCTVMapScreen> {
  ...
  Future<void> _createSafeRoutes() async {
    if (_currentLocation == null) return;

    LatLng currentLatLng = LatLng(_currentLocation!.latitude!, _currentLocation!.longitude!);
    List<LatLng> cctvLocations = await _fetchCCTVLocations();
    ...
    for (int i = 0; i < 4; i++) {
      ...
      do {
        waypoints = _generateRandomWaypoints(currentLatLng, 0.8, 1.5, 3);
        routeDistanceKm = await _createRouteWithWaypoints(currentLatLng, waypoints);
        List<LatLng> fullRoute = [currentLatLng, ...waypoints, currentLatLng];
        isSafe = _isRouteSafe(fullRoute, cctvLocations);

        retryCount++;
      } while ((!isSafe || routeDistanceKm < 3 || routeDistanceKm > 4) && retryCount < 10);
      if (retryCount >= 10 && (!isSafe || routeDistanceKm < 3 || routeDistanceKm > 4)) {
        continue;
      }
    }
  }
  ...
  Future<List<LatLng>> _fetchCCTVLocations() async {

    return [];
  }


  bool _isRouteSafe(List<LatLng> route, List<LatLng> cctvLocations) {
    const double safeDistanceMeters = 100.0;
    return route.every((point) => cctvLocations.any((cctv) {
      return _calculateDistance(point, cctv) <= safeDistanceMeters / 1000;
    }));
  }
